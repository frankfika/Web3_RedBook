...
| Bootstrapping                             | 引导启动）通常指的是一个项目或加密货币的初始阶段，尤其是在没有大规模投资或外部支持的情况下自行启动和运作的过程。具体来说，Bootstrapping 可能包括以下方面： 自主融资： 项目的创始人或团队可能通过自己的资金，或者通过小规模的私人投资来启动项目，而不依赖于传统的风险投资。 社区建设： 在加密货币领域，一些项目可能通过社区的支持和参与来启动。这包括社区成员的贡献，参与矿工、节点等网络活动。 有限资源下的发展： Bootstrapping 意味着在有限的资源下开始发展项目，通过逐步增加规模和吸引更多投资来实现项目的目标 |
...
| Contract                                  | 合约                                                         |
| GWei                                      | "Gwei" 是以太坊网络中用于表示以太币（Ether）的最小单位之一。它是以太币的一千万分之一，类似于比特币网络中的 "satoshis"，是比以太币更小的可分割单位。 具体来说，1 Ether = 1,000,000,000 Gwei。 在以太坊的智能合约和交易中，通常使用 Gwei 来表示燃气费用（Gas Fee）。燃气费用是在以太坊网络上执行智能合约和交易所需的费用，它以 Gwei 为单位，用户需要支付一定数量的 Gwei 作为矿工费用，以确保他们的交易能够被矿工打包并添加到区块链中。 |
...
| 撸空投    |                                                              |
| CX        |                                                              |
| DYOR      | Do your own research意思是你要自己做研究来决定是否购买，是一个免责声明。 |
| FOMO      | Fear of missing out，意思是害怕失去机会。在这种情绪下，往往会高价接盘，容易被套牢。 |
